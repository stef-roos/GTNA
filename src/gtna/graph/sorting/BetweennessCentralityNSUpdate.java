 ...
 *
 * ---------------------------------------
 * BetweennessCentralityNSUpdate.java
 * ---------------------------------------
 * (C) Copyright 2009-2011, by Benjamin Schiller (P2P, TU Darmstadt)
 * and Contributors 
 *
 ...
 *
 */
public class BetweennessCentralityNSUpdate extends NodeSorterUpdate{
	private Graph g;
	...
	*/
	...
	for (Node n : nodes) {
		if (!removed[n.getIndex()]){
	this.bcMap.put(n, 0.0);
		} else {
			this.bcMap.put(n, Double.MAX_VALUE);
		}
	}

	for (Node s : nodes) {
		if (removed[s.getIndex()])continue;
	// empty stack
	...
	for (Node n : nodes) {
		if (removed[n.getIndex()])continue;
...
//	// -----
	...
	* (1 + delta.get(w).doubleValue()));
	...
	double point1 = bcMap.get(n1).doubleValue();
	double point2 = bcMap.get(n2).doubleValue();
	int result;
	if (point1 == point2) {
	result = 0;
	...
	*/
	...
	return bcMap.get(n1).doubleValue() == bcMap.get(n2).doubleValue();
	}
