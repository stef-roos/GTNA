...
public abstract class FragmentationRecompute extends Metric {
	...
	public FragmentationRecompute(Type type, NodeSorterUpdate sorter, Resolution resolution) {
	...
	public void computeData(Graph g, Network n, HashMap<String, Metric> m) {
		...
		for (int i = 0; i < excludeFirst.length; i++) {
			exclude = this.getExclude(sorted, exclude,excludeFirst[i],i>0?excludeFirst[i-1]:0, rand);
	...
	private boolean[] getExclude(Node[] sorted, boolean[] exclude,int excludeFirst, int startExclude,
			Random rand) {
		for (int i = startExclude; i < excludeFirst; i++) {
			exclude[sorted[i].getIndex()] = true;
			this.sorter.update(exclude, i,rand);
		}
		return exclude;
	}
