 ...
 *
 * ---------------------------------------
 * CommunityDetectionLPAClustering.java
 * ---------------------------------------
 * (C) Copyright 2009-2011, by Benjamin Schiller (P2P, TU Darmstadt)
 * and Contributors 
 *
 ...
 *
 */
public class CommunityDetectionLPAClustering extends Transformation {
	...
	private int[] labelPropagationAlgorithm(Node[] nodes) {
		...
		double[][] weights = this.initWeights(nodes);
		...
		while (!finished) {
			...
			for (Node x : X) {
				ArrayList<Integer> maxLabels = selectMaxLabels(
						x.getOutgoingEdges(), labels, weights[x.getIndex()]);
	 ...
	 */
	private ArrayList<Integer> selectMaxLabels(int[] nodes, int[] labels, double[] weights) {
		HashMap<Integer, Double> counter = new HashMap<Integer, Double>(); // <label,
																				// count>
		ArrayList<Integer> maxLabels = new ArrayList<Integer>();
		double maxCount = 0;

		for (int  i = 0;i < nodes.length; i++) {
			int label = labels[nodes[i]];
			double count = counter.containsKey(label) ? counter.get(label) + weights[i] : weights[i];
	...
	private double[][] initWeights(Node[] nodes){
		...
		for (int i = 0; i < nodes.length; i++){
			...
			for (int j=0; j < out.length; j++){
				...
				for (int n2: out){
					if (n1 != n2){
						if (nodes[n1].hasNeighbor(n2)){
							count1++;
						}
					}
				}
				count2 = 0;
				out2 = nodes[n1].getOutgoingEdges();
				for (int n2: out2){
					if (i != n2){
						if (nodes[i].hasNeighbor(n2)){
							count2++;
						}
					}
				}
				weights[i][j] = count1/(double)(nodes[i].getInDegree()-1); 
				if (nodes[n1].getInDegree() > 1){
				   weights[i][j] += count2/(double)(nodes[n1].getInDegree()-1);
				}		
			}
		}
