 ...
 *
 */
public class KleinbergDegreeDist extends Network {

	private int SHORT_RANGE_CONTACTS;
	private double[] LONG_RANGE_CONTACTS;
	 ...
	 * @param t
	 */
	public KleinbergDegreeDist(int nodes, int SHORT_RANGE_CONTACTS,
			double[] LONG_RANGE_CONTACTS, String ddist,double CLUSTERING_EXPONENT,
			boolean BIDIRECTIONAL, boolean RANDOM, Transformation[] t) {
		super("KLEINBERG_DEGREE_DIST", nodes,
				new Parameter[] {
						new IntParameter("SHORT_RANGE_CONTACTS",
								SHORT_RANGE_CONTACTS),
						new StringParameter("LONG_RANGE_CONTACTS",
								ddist),
	...
	public Graph generate() {
		...
		Edges edges = new Edges(nodes, this.getNodes()
				* (this.SHORT_RANGE_CONTACTS * 2 + this.LONG_RANGE_CONTACTS.length));
	...
	private void generateLongRangeContacts(double sum,
		...
		int l = 0;
		double s = this.LONG_RANGE_CONTACTS[0];
		double r = rand.nextDouble();
		while (s < r){
			l++;
			s = s + this.LONG_RANGE_CONTACTS[l];
		}
		double[] rands = new double[l];
