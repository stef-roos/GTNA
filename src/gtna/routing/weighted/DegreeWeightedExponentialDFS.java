 ...
 *
 * ---------------------------------------
 * DegreeWeightedExponentialDFS.java
 * ---------------------------------------
 * (C) Copyright 2009-2011, by Benjamin Schiller (P2P, TU Darmstadt)
 * and Contributors 
 *
 ...
 *
 */
public class DegreeWeightedExponentialDFS extends NodeGreedy {
	
	public DegreeWeightedExponentialDFS() {
		super("DEPTH_WEIGHTED_EXPONENTIAL_DFS");
	}

	public DegreeWeightedExponentialDFS(int ttl) {
		super(ttl, "DEPTH_WEIGHTED_EXPONENTIAL_DFS");
	}
	...
	public int getNextD(int current, DIdentifier target, Random rand,
		...
		for (int out : nodes[current].getOutgoingEdges()) {
			double dist = Math.pow(this.pD[out].distance(target), 1/(double)nodes[out].getDegree());
